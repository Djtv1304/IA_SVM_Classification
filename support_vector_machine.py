...
# Cargando el dataset
dataset = pd.read_csv('birthwt_data_modified.csv')  # Lee el archivo CSV con los datos
...
# Predicción de un nuevo resultado (Ejemplo: Edad = 24, Peso = 3200)
resultado = classifier.predict(sc.transform([[24, 3200]]))  # Predice si comprará o no
print(f"Predicción para edad 24 y el peso de 3200: {resultado}")
...
plt.xlabel('Age o Edad')
plt.ylabel('Birthday Weight (BWT)')
plt.legend()
plt.show()
